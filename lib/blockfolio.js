...
(function () {
    ...
    class Blockfolio {
        ...
        constructor(fiatCurrency, apiUrl, magicToken, locale, platform) {
            this.CLIENT_TOKEN = null;
            this.FIAT_CURRENCY = fiatCurrency;
            this.LOCALE = locale;
            this.PLATFORM = platform;
            this.utils = new BlockUtils(apiUrl, magicToken);
            this.coins = [];
        }
        ...
        _refreshCoins(callback) {
            this.getCoinsList((err, coins) => {
                ...
                const fCoins = {};
                coins.forEach((coin) => {
                    if (typeof fCoins[coin.aliasTokenSymbol] == "undefined") {
                        fCoins[coin.aliasTokenSymbol] = {
                            name: coin.fullName,
                            bases: [coin.base]
                        };
                    } else {
                        fCoins[coin.aliasTokenSymbol].bases.push(coin.base);
                    }
                });
                this.coins = fCoins;
                callback(null);
         ...
         */
        createNew(callback) {
            const token = parseInt(Math.random().toString().substring(2)).toString(12); // Generate random device token
            this._register(token, (err) => {
                if (err) { return callback(err); }

                this.init(token, (err, Blockfolio) => {
                    if (err) { return callback(err); }

                    return callback(null, Blockfolio);
                });
            })
        }
        ...
        _validateTokenPair(pair, callback) {
            ...
            if (typeof this.coins[pair.token] === "undefined") {
                callback(new Error(`${pair.token} is not an available token on Blockfolio!`));
            }

            if (this.coins[pair.token].bases.indexOf(pair.base) < 0) {
                callback(new Error(`${pair.token} is not an available in ${pair.base} on Blockfolio!`),
        ...
        getHoldings(pair, callback) {
            if (!this._checkClientToken()) {
                return callback(new Error("A valid CLIENT_TOKEN should be provided"));
            }
