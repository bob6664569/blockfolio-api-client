        ...
        addCoin() {
            ...
            let addDefault = ({
                ...
                this._validateTokenPair(arguments[0], (err, pair) => {
                    ...
                    let doCall = () => {
                        self.utils.call(`add_position_v2/${self.CLIENT_TOKEN}/${mode}/${pair.token}/${pair.base}/${exchange}/${price}/${amount}/${timestamp}/0?platform=${self.PLATFORM}&note=${note}&use_alias=true`, (err) => {
                            if (err) { return (callback) ? callback(err) : reject(err); }
                    ...
                    let checkPrice = () => {
                        if (price == null) {
                            self.getPrice(pair, exchange, (err, rPrice) => {
                                if (err) { return (callback) ? callback(err) : reject(err); }
                    ...
                    if (exchange === null) {
                        self.getExchanges(pair, (err, exchanges) => {
                            if (err) { return (callback) ? callback(err) : reject(err); }
            ...
            // Handle args
            if (typeof arguments[1] === "function") { // Only Coin, No option provided
                callback = arguments[1];
                addDefault({});
            } else if (typeof arguments[2] === "function") { // Coin + Options + Callback
                callback = arguments[2];
                addDefault(arguments[1]);
            } else {// arguments.length === 1 -> return Promise
                callback = false;
                const promise = new Promise((_resolve, _reject) => {
                    resolve = _resolve;
                    reject = _reject;
                });
                if (arguments.length == 2) { // Coin + Options
                    addDefault(arguments[1]);
                } else {
                    addDefault();
                }
