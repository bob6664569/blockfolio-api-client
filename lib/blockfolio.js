...
(function () {
    ...
    class Blockfolio {
        ...
        addPosition() {
            return this.utils._handlePromises(arguments, 1, ({
                ...
                this._validateTokenPair(arguments[0]).then((pair) => {
                    ...
                    const doCall = () => {
                        this.utils.call(`add_position_v2/${this.CLIENT_TOKEN}/1/${pair.token}/${pair.base}/${exchange}/${price}/${amount}/${timestamp}/0?platform=${this.PLATFORM}&note=${note}&use_alias=true`).then(() => {
                            return callback();
        ...
        getAlerts() {
            ...
            return this.utils._handlePromises(arguments, 1, ({} = {}, callback) => {
                this._validateTokenPair(arguments[0]).then((pair) => {
                    this.utils.call(`get_alerts/${this.CLIENT_TOKEN}?coin=${pair.token}&base=${pair.base}&locale=${this.LOCALE}&use_alias=true`).then((pBody) => {
         ...
         * @param {Number}          options.under       Lower price to trigger the alert
         * @param {Boolean}         options.persistant  If let to false, alert triggers only once
        ...
        addAlert() {
            ...
            return this.utils._handlePromises(arguments, 1, ({
                ...
                if (above === 0 && under === 0) { return callback(new Error("You must specify at leat a boundary to set up an alert!")); }
                this._validateTokenPair(arguments[0]).then((pair) => {
                    const doCall = () => {
                        persistant = persistant ? 1 : 0;

                        this.utils.call(`add_alert/${this.CLIENT_TOKEN}/5/${pair.token}/${pair.base}/${exchange}/${above}/${under}/${persistant}?platform=${this.PLATFORM}&use_alias=true`).then((pBody) => {
                            return callback();
                        }).catch((err) => {
                            return callback(err);
                        });
        ...
        removeAlert() {
            return this.utils._handlePromises(arguments, 1, ({} = {}, callback) => {
                ...
                this.utils.call(`remove_alert/${this.CLIENT_TOKEN}/${arguments[0]}?use_alias=true`).then(() => {
                    return callback();
                }).catch((err) => { return callback(err); });
            });
        }
        ...
        pauseAlert() {
            return this.utils._handlePromises(arguments, 0, ({} = {}, callback) => {
                ...
                this.utils.call(`pause/${this.CLIENT_TOKEN}/${arguments[0]}?use_alias=true`).then(() => {
                    return callback();
                }).catch((err) => { return callback(err); });
            });
        }
        ...
        startAlert() {
            return this.utils._handlePromises(arguments, 0, ({} = {}, callback) => {
                ...
                this.utils.call(`start/${this.CLIENT_TOKEN}/${arguments[0]}?use_alias=true`).then(() => {
                    return callback();
                }).catch((err) => { return callback(err); });
            });
        }
        ...
        startAllAlerts() {
            return this.utils._handlePromises(arguments, 1, ({} = {}, callback) => {
                ...
                this._validateTokenPair(arguments[0]).then((pair) => {
                    this.utils.call(`start_all/${this.CLIENT_TOKEN}/${pair.token}/${pair.base}?use_alias=true`).then(() => {
                        return callback();
                    }).catch((err) => { return callback(err); })
                }).catch((err) => { return callback(err); });
            });
        ...
        pauseAllAlerts() {
            return this.utils._handlePromises(arguments, 1, ({} = {}, callback) => {
                ...
                this._validateTokenPair(arguments[0]).then((pair) => {
                    this.utils.call(`pause_all/${this.CLIENT_TOKEN}/${pair.token}/${pair.base}?use_alias=true`).then(() => {
                        return callback();
                    }).catch((err) => { return callback(err); })
                }).catch((err) => { return callback(err); });
            });
         ...
         */
        getPositions() {
            return this.utils._handlePromises(arguments, 0, ({
                pair = null
            } = {}, callback) => {
                ...
                if (pair === null) {
                    ...
                    this._validateTokenPair(pair).then((pair) => {
