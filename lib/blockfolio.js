...
(function () {
    ...
    class Blockfolio {
         ...
         * @private
         */
        _handlePromises(args, callback) {
            let fCallback = args[args.length - 1];
            const checkArgsAndGo = (nMinArgs, options = {}) => {
         ...
         */
        addPosition() {
            return this._handlePromises(arguments, ({
                mode        = "buy",
                amount      = 0,
                note        = "",
                ...
                this._validateTokenPair(arguments[0], (err, pair) => {
                    ...
                    const doCall = () => {
                        this.utils.call(`add_position_v2/${this.CLIENT_TOKEN}/${mode}/${pair.token}/${pair.base}/${exchange}/${price}/${amount}/${timestamp}/0?platform=${this.PLATFORM}&note=${note}&use_alias=true`, (err) => {
                            if (err) { return callback(err); }
