        ...
        _validateTokenPair() {
            return this.utils._handlePromises(arguments, 1, ({} = {}, callback) => {
                ...
                this.coins.forEach((coin) => {
                    if ((coin.base === pair.base) && (coin.token === pair.aliasTokenSymbol)) {
            ...
            return this.utils._handlePromises(arguments, 1, ({
                ...
                this._validateTokenPair(arguments[0]).then((pair) => {
                    const doCall = () => {
                        ...
                        this.utils.call(`add_alert/${this.CLIENT_TOKEN}/5/${pair.token}/${pair.base}/${exchange}/${above}/${under}/${persistant}?platform=${this.PLATFORM}&use_alias=${this.USE_ALIAS}`).then((pBody) => {
                            return callback();
                        }).catch((err) => {
                            return callback(err);
                        });
