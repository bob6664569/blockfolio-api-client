...
(function () {
    ...
    class Blockfolio {
        ...
        getPrice(pair, exchange, callback) {
            ...
            this.utils.call(`lastprice/${exchange}/${pair.base}-${pair.token}?locale=${this.LOCALE}&use_alias=true`, (err, pBody) => {
        ...
        getCoinsList(callback) {
            this.utils.call(`coinlist_v6`, (err, pBody) => {
                ...
                if (typeof pBody.coins == "undefined") return callback(new Error("Bad response"));
        ...
        removeCoin(pair, callback) {
            ...
            pair = this.utils.parseToken(pair);
            this.utils.call(`remove_all_positions/${this.CLIENT_TOKEN}/${pair.base}-${pair.token}?use_alias=true`, (err, pBody) => {
                if (err) return callback(err);
