...
(function () {
    ...
    class Blockfolio {
        ...
        init() {
            return this.utils._handlePromises(arguments, 1, ({
                ...
                this._refreshCoins().then(() => {
                    return callback();
                }).catch(this._handleErrors(callback));
            });
        }
        ...
        getVersion() {
            return this.utils._handlePromises(arguments, 0, ({} = {}, callback) => {
                this.utils.call(`version?platform=${this.PLATFORM}`).then((pBody) => {
        ...
        getStatus() {
            return this.utils._handlePromises(arguments, 0, ({} = {}, callback) => {
                this.utils.call(`system_status?platform=${this.PLATFORM}`).then((pBody) => {
        ...
        getExchanges() {
            return this.utils._handlePromises(arguments, 1, ({} = {}, callback) => {
                this._validateTokenPair(arguments[0]).then((pair) => {
                    this.utils.call(`exchangelist_v2/${pair.base}-${pair.token}`).then((pBody) => {
        ...
        getPortfolioSummary() {
            return this.utils._handlePromises(arguments, 0, ({} = {}, callback) => {
                ...
                this.utils.call(`get_all_positions/${this.CLIENT_TOKEN}?use_alias=${this.USE_ALIAS}&fiat_currency=${this.FIAT_CURRENCY}`).then((pBody) => {
        ...
        getPositions() {
            ...
            return this.utils._handlePromises(arguments, offset, ({} = {}, callback) => {
                ...
                if (offset === 0) {
                    ...
                    this._validateTokenPair(arguments[0]).then((pair) => {
                        this.utils.call(`get_positions_v2/${this.CLIENT_TOKEN}/${pair.base}-${pair.token}?use_alias=${this.USE_ALIAS}&fiat_currency=${this.FIAT_CURRENCY}&locale=${this.LOCALE}`).then((pBody) => {
                            if (typeof pBody.positionList == "undefined") {
                                return callback(new Error("Bad response"));
                            }
        ...
        getCurrencies(callback) {
            return this.utils._handlePromises(arguments, 0, ({} = {}, callback) => {
                this.utils.call("currency").then((pBody) => {
        ...
        getHoldings() {
            return this.utils._handlePromises(arguments, 1, ({} = {}, callback) => {
                ...
                this._validateTokenPair(arguments[0]).then((pair) => {
                    this.utils.call(`get_combined_position/${this.CLIENT_TOKEN}/${pair.base}-${pair.token}?use_alias=${this.USE_ALIAS}&fiat_currency=${this.FIAT_CURRENCY}&locale=${this.LOCALE}`).then((pBody) => {
        ...
        getCoinsList(callback) {
            return this.utils._handlePromises(arguments, 0, ({} = {}, callback) => {
                this.utils.call("coinlist_v6").then((pBody) => {
        ...
        removeCoin() {
            return this.utils._handlePromises(arguments, 1, ({} = {}, callback) => {
                ...
                this._validateTokenPair(arguments[0]).then((pair) => {
                    this.utils.call(`remove_all_positions/${this.CLIENT_TOKEN}/${pair.base}-${pair.token}?use_alias=${this.USE_ALIAS}`).then((pBody) => {
                        return callback(null, pBody);
                    });
                }).catch(this._handleErrors(callback));
            });
        ...
        getMarketDetails() {
            return this.utils._handlePromises(arguments, 1, ({
                ...
                const doCall = (pair) => {
                    this.utils.call(`marketdetails_v2/${this.CLIENT_TOKEN}/${exchange}/${pair.base}-${pair.token}?use_alias=${this.USE_ALIAS}&locale=${this.LOCALE}&fiat_currency=${this.FIAT_CURRENCY}`).then((pBody) => {
                        return callback(null, pBody);
                    });
                };
