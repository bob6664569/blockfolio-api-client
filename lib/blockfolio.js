...
(function () {
    ...
    class Blockfolio {
         ...
         * @callback
         */
        addPosition(buy, pair, exchange, initPrice, amount, date, note, callback) {
            if (!this._checkClientToken()) {
                return callback(new Error("A valid CLIENT_TOKEN should be provided! (Have you called Blockfolio.init()?)"));
            }

            // Prepare params
            amount = buy ? Math.abs(amount) * -1 : Math.abs(amount);
            ...
            this._validateTokenPair(pair, (err, pair) => {
                ...
                // Fetch add
                this.utils.call(`add_position_v2/${this.CLIENT_TOKEN}/1/${pair.token}/${pair.base}/${exchange}/${initPrice}/${amount}/${timestamp}/0?platform=${this.PLATFORM}&note=${URLEncodedNote}&use_alias=true`, (err, pBody) => {
                    if (err) { return callback(err); }
        ...
        watchCoin(pair, exchange, callback) {
            return this.addPosition(1, pair, exchange, 0, 0, null, "", callback);
        }
